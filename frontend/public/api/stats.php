...
if ($leadsDb) {
    ...
    foreach (['utm_source', 'utm_campaign', 'gclid', 'fbclid', 'referrer', 'landing_page', 'address', 'occupancy', 'units', 'condition', 'timeframe', 'price', 'contact_pref', 'updated_at', 'crm_id', 'webhook_status', 'webhook_last_error'] as $c) {
...
if ($leadsDb && $sameOrigin && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && isset($_POST['id'], $_POST['status']) && in_array($_POST['status'], $statuses, true)) {
    $leadsDb->prepare('UPDATE leads SET status = ?, updated_at = ? WHERE id = ?')->execute([$_POST['status'], gmdate('Y-m-d\\TH:i:s\\Z'), (int)$_POST['id']]);
...
$list = $q($leadsDb, "SELECT id, created_at, type, location, name, email, phone, message, locale, source, status, utm_source, utm_campaign, gclid, fbclid, referrer, landing_page, address, occupancy, units, condition, timeframe, price, contact_pref, crm_id, webhook_status, webhook_last_error FROM leads ORDER BY id DESC LIMIT 100");
...
<tr><th>#</th><th>Datum</th><th>Type</th><th>Locatie</th><th>Naam</th><th>Contact</th><th>Bron</th><th>CRM</th><th>Status</th></tr>
...
<td><?php $ws = $r['webhook_status'] ?: 'pending'; ?><span class="pill" style="background:<?= $ws === 'delivered' ? '#E3F4EA' : ($ws === 'failed' ? '#FDE8E7' : '#FFF4E0') ?>" title="<?= $h($r['webhook_last_error']) ?>"><?= $h($ws) ?></span><?php if ($r['crm_id']): ?><div class="muted"><?= $h($r['crm_id']) ?></div><?php endif ?></td>
