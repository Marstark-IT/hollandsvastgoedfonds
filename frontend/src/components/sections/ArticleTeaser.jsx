    ...
    <section className={`${tone === "soft" ? "bg-soft" : ""} py-20 md:py-28`}>
      <div className="wrap">
        ...
        <ul className="mt-10 grid gap-6 md:grid-cols-3">
          {list.map((art) => (
            <li key={art.slug}>
              <Link href={articlePath(art.slug)} className="group flex h-full flex-col overflow-hidden rounded border border-line bg-white">
                <div className="relative aspect-[16/10] overflow-hidden">
                  <Image src={IMAGES[art.image]} alt="" fill sizes="(min-width:768px) 33vw, 100vw" className="object-cover transition-transform duration-500 group-hover:scale-[1.04]" />
                </div>
                <div className="flex flex-1 flex-col p-6">
                  ...
                  <p className="mt-3 line-clamp-3 text-[0.98rem] text-muted">{art.description}</p>
                </div>
              </Link>
            </li>
          ))}
